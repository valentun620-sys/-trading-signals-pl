...
const I18N = {
    ...
    pl: {
        ...
        field_model_ph: "Wybierz model",
        btn_get_signal: "Pobierz sygnał",

        signal_title: "Sygnał",
        signal_model_prefix: "Model:",
        steps_1_t: "Screening techniczny",
        ...
        k_time: "CZAS",
        k_valid: "WAŻNE DO",
        v_strength_high: "Silna",
        v_strength_medium: "Średnia",
        v_vol_low: "Niski",
        v_vol_medium: "Średni",
        v_vol_high: "Wysoki",
        v_market_otc: "OTC",
        ...
        faq_title: "FAQ",
        faq_q1: "Co to jest sygnał AI?",
        faq_a1: "Sygnał generowany przez sztuczną inteligencję na podstawie analizy rynku.",
        faq_q2: "",
        faq_a2: "",
        faq_q3: "Co to jest czas wygaśnięcia?",
        faq_a3: "Moment, w którym transakcja zostaje automatycznie zamknięta.",
        faq_q4: "Czym różnią się modele?",
        faq_a4: "Modele różnią się wydajnością i mocą obliczeniową.",
        faq_q5: "Czy sygnały są dokładne?",
        faq_a5: "Sygnały są agregowane z wielu rynków dla większej stabilności.",

        cp_title_fiat: "Waluty",
        cp_title_crypto: "Krypto",
        cp_title_commod: "Towary",
        cp_title_stocks: "Akcje",
        cp_title_docs: "Indeksy",
        cp_title_fav: "Ulubione",
        cp_title_search: "Szukaj",
        cp_search_ph: "Szukaj",
        cp_fav_only_title: "Tylko ulubione",
        cp_head_market: "Rynek",
        cp_empty: "Nic nie znaleziono",

        ex_title: "Czas wygaśnięcia",
        md_title: "Model AI",
        md_v1: "NeuralEdge v2.0",
        md_v2: "NeuralEdge v2.0",
        md_vip_note: "",

        vip_sheet_title: "Uzyskaj dostęp VIP",
        vip_close_aria: "Zamknij",
        vip_hero_badge: "Ekskluzywny dostęp",
        vip_hero_h4: "Większa dokładność. Więcej rynków.",
        vip_hero_sub: "VIP odblokowuje zaawansowany model i priorytetowe sygnały.",
        vip_compare_free: "Podstawowy",
        vip_compare_vip: "VIP",
        vip_free_list: [],
        vip_paid_list: [],
        vip_h3: "Co daje VIP",
        vip_feat_1_t: "+ do 12% ↑ dokładności",
        vip_feat_1_s: "dzięki ansamblom.",
        vip_feat_2_t: "Dostawa priorytetowa",
        vip_feat_2_s: "VIP otrzymuje sygnał jako pierwszy.",
        vip_feat_3_t: "Przewodniki ryzyka",
        vip_feat_3_s: "adaptacyjna wielkość pod zmienność.",
        vip_feat_4_t: "Rozszerzone rynki",
        vip_feat_4_s: "pakiety akcji, indeksów i krypto.",
        vip_how_h4: "Jak uzyskać VIP",
        vip_how_p: "Handluj na PocketOption. VIP przyznawany jest automatycznie za obrót.",
    },
...
function applyI18nToDOM() {
    ...
    // ТЕ ПАРЫ КОТОРЫЕ БЫЛИ НА АНГЛИЙСКОМ (CONFIDENCE / ACCURACY)
    const counterLabels = document.querySelectorAll(".rg2-counter .k");
    if (counterLabels[0]) counterLabels[0].textContent = t("k_conf");
    if (counterLabels[1]) counterLabels[1].textContent = t("k_acc");
    ...
    faqItems.forEach((it, i)=>{
        ...
        if (!q) {
            ...
            it.querySelector(".faq-question span")?.replaceChildren(q);
...
// Init language
renderLangMenu();
setCurrentLang(localStorage.getItem(LANG_KEY) || "pl");
...
(function(){
    ...
    function setExpiryUI(item){
        ...
        const targets = [document.getElementById("timeField")].filter(Boolean);
        targets.forEach(el => {
...
(function(){
    ...
    function render(){
      ...
      <button class="md-chip${m.disabled ? " is-disabled" : ""}" data-id="${m.id}" ${m.disabled ? 'disabled' : ""} aria-selected="${m.label===selectedId}">
        <span>${m.label}</span>
      </button>
    `).join("");
    ...
    function setModelUI(item){
        ...
        const mf = document.getElementById("modelField"); if (mf) mf.value = item.label;
        const sm = document.getElementById("selectedModel"); if (sm) sm.textContent = item.label;
        checkReady();
        saveState();
    }
...
(function(){
    ...
    function showResult(pair, time){
        ...
        const acc  = Math.floor(40 + Math.random()*48);
        const market = /OTC/i.test(pair) ? "OTC" : "REAL";

        let valid = "—";
        try{
            const secs = state.expirySeconds || 300;
        ...
        document.getElementById("sigStrength").textContent = i18nFormatStrength("High");
        ...
        document.getElementById("sigVol").textContent      = i18nFormatVolume("Medium");

        document.getElementById("sigAnalysis").style.display = "none";
        document.getElementById("sigResult").hidden = false;

        saveResult({ isBuy, pair, conf: conf + "%", acc: acc + "%", market, time, valid });
    }
    ...
    function resetAll(){
        ...
        resetSigSteps();
        state.pair = null; state.time = null;
        document.getElementById("pairField").value = "";
...
function setSigStepsState(currentIndex){
    ...
    if(_sigBar) _sigBar.style.width = Math.min(100, (currentIndex / _sigSteps.length) * 100) + "%";
}
function resetSigSteps(){ _sigSteps.forEach(el=>el.classList.remove("is-done","is-active")); }
